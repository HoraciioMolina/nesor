#!/usr/bin/env node
// prepublishOnly guard for @nesor/mapper.
//
// peerDependencies.nesor is declared as "workspace:^" in the source package.json
// because this is a pnpm workspace. `pnpm publish` rewrites that to a concrete
// version before packing; `npm publish` does NOT — it ships the literal string
// "workspace:^", which breaks consumers.
//
// This script refuses to publish from non-pnpm tooling, so we never ship a
// broken tarball. Run via `pnpm publish` (preferred) or `pnpm release` from the
// monorepo root.
